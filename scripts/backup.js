...
window.addEventListener("load", async () => {
    ...
    warnings(jsondata), dataLoad(bIndex);
    ...
    (!windowsFilenameIllegalCharacters(e.key) || 50 < elName.value.length) && e.preventDefault();
    ...
    150 < elMsgBefore.value.length && e.preventDefault();
    ...
    150 < elMsgAfter.value.length && e.preventDefault();
...
function active(active) {
    fields.forEach(fld => {
        fld !== elActive ? (active ? elDate.checked && "includedate" !== !fld.getAttribute("name") || fld.removeAttribute("disabled") : (warningsRemove(),
...
function fileLineIndexNew() {
    ...
    let a = arrIndexes.reduce((acc, cv) => acc < cv ? cv : acc, 0);
    return a + 1;
}
...
function fileLineIndexToLineNumber(index) {
    ...
    let i = 0;
    for (;i += 1, Number(elTemp[i].getAttribute("data-index")) !== index && 200 !== i; ) ;
    return i;
}
...
function dateDisplay(fieldDate) {
    ...
    var today;
    if ("undefined" == typeof cd && (today = new Date(), cd = today.getDate(), cm = today.getMonth(),
    cy = today.getFullYear()), 0 === fieldDate.length) return "";
    var d = new Date(fieldDate);
    let day = d.getDate(), mnth = [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ][Number(d.getMonth())], yr;
    return cy === d.getFullYear() ? (yr = "", d < today ? numberOfNightsBetweenDates(d, today) < 7 && (0 === numberOfNightsBetweenDates(d, today) ? (day = "Today",
    mnth = "") : 1 === numberOfNightsBetweenDates(d, today) && (day = "Yesterday", mnth = "")) : numberOfNightsBetweenDates(today, d) < 7 && (mnth = (day = 0 === numberOfNightsBetweenDates(today, d) ? "Today" : 1 == numberOfNightsBetweenDates(today, d) ? "Tomorrow" : function(i) {
        return [ "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" ][i];
    }(d.getDay()), ""))) : yr = d.getFullYear(), String(day + ("" == mnth ? "" : " " + mnth) + ("" == yr ? "" : " " + yr));
}
...
function fileLineActive(index, value) {
    ...
    !scriptRootDirDate && value ? document.querySelector(`.filelist__line[data-index="${index}"] .filelist__date input`).removeAttribute("disabled") : document.querySelector(`.filelist__line[data-index="${index}"] .filelist__date input`).setAttribute("disabled", !0);
}

elFileAdd.addEventListener("click", function() {
    let dataindex = fileLineIndexNew();
    fileLineAdd(dataindex - 1), jsondata["Backup List"][bIndex].Files.push({
        "File Or Folder": "",
        "File Type": "",
        "Zip It": !1,
        "Sub-Directories": !1,
    ...
    let bID = Number(document.querySelector(".backupnamelist tr.selected").getAttribute("data-id")), bIndex = backupIDToIndex(bID);
