					...
					parseJsonAsync(strFileContent)
						.then((objJSON) => {
							if (objJSON['Backup List']
								.length !== 0) {
								const backupListID =
									backupListFindFirstID(
										objJSON)
								delete objJSON
									.BackupListID
								objJSON.BackupListID =
									Number(backupListID)
								const
	...
	return new Promise((resolve, reject) => {
		...
		parseJsonAsync(strNew)
			.then((jsontemplate) => {
				...
				fs.writeFile(settingsFile, strjson, function(
					err) {
					if (err) {
						console.log(err)
						reject(
...
async function buildErrorChecker(jsonobj) {
	...
	for (let i = 0; i < json['Backup List'].length; i++) {
		...
		try {
			...
			console.error(error)
			json['Error List'][i]['Backup Name'] =
				"Script file doesn't exist."
		}

		//* Backup Root Directory
		try {
			await fsp.access(json['Backup List'][i][
				'Backup Root Directory'
			])
		} catch (error) {
...
async function powershellFileWrite(fileName, fileText) {
	fs.writeFile(fileName, fileText, function(err) {
		if (err) {
			...
			console.log('Backup Powershell script ' + fileName +
				' written to')
			resolve(fileText)
		}
	})
